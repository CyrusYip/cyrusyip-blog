...
tags:
    - driving
lastmod: 2026-04-08T22:12:34+08:00
---
...
1. **同方向挡位切换不需要踩刹车**，比如：D 挡到 M 挡或者 L 挡（都是前进）。不同方向挡位切换要踩刹车（因为在行进时突然变方向会伤车），比如从 D 挡到 R 挡（前进到倒退）。
1. **进入 N 挡不需要踩刹车**（车子行进时不要挂 N 挡，N 挡没有发动机制动，不安全）。开车的时候如果不小心挂到 N 挡，踩着油门时车的转速会飙升，此时松油门挂回 D 挡就好，不需要刹停。
